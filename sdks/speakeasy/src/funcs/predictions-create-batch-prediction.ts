 ...
 */
export function predictionsCreateBatchPrediction(
  client: SDKCore,
  request: operations.CreateBatchPredictionRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    models.BatchPredictionResponse,
    | SDKError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.CreateBatchPredictionRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.BatchPredictionResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
